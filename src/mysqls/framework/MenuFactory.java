...
public class MenuFactory {
    ...
    /**
     * @param pBundle The bundle to use to fetch resources.
     */
     ...
     *
     * @param pPrefix     A string such as "file.open" that indicates the menu->submenu
     *                    path
     * @param pTarget     The object on which pMethodName will be invoked when the menu
     *                    is selected.
    ...
    private JMenuItem configure(JMenuItem pMenuItem, String pPrefix, ActionListener pListener) {
        ...
        if (aBundle.containsKey(pPrefix + ".icon")) {
            pMenuItem.setIcon(
                    new ImageIcon(getClass().getClassLoader().getResource(aBundle.getString(pPrefix + ".icon"))));
        }
        return pMenuItem;
    }
