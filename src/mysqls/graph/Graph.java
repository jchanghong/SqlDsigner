...
public abstract class Graph {
	protected GraphModificationListener aModificationListener; // Only access
																// from notify*
																// methods and
																// setter
	...
	/**
	 * Notifies the listener, if applicable, of a change to a property of one of
	...
	/**
	 * @return A short description of this diagram, usually ending in "Diagram",
	 *         e.g., "State Diagram".
	 */
	...
	/**
	 * Adds a newly created node to the graph so that the top left corner of the
	 * bounding rectangle is at the given point. This method is intended to be
	 * used to add nodes that were never part of the graph, from the GUI. To add
	 * nodes recovered from deserialization, use restoreNode. To add nodes
	 * recovered from in-application operations such as undoing and pasting, use
	 * insertNode. This method assumes the node does not have a parent of a
	 * child.
	 *
	 * @param pNode
	 *            the node to add
	...
	public boolean addNode(Node pNode, Point2D pPoint) {
		...
		// if( !(pNode instanceof ChildNode) || ((ChildNode)pNode).getParent()
		// == null )
		// {
	...
	/**
	 * Inserts a previously-created node back into a diagram. The node is
	 * expected to have a position and possibly a parent. Node insertion is
	 * intended to support operations such as undo/redo and copy and paste.
	 * Calling this method results in a node addition notification.
	 *
	 * @param pNode
	 *            The node to insert into the graph.
	 */
	...
	/**
	 * Removes all edges in the graph that have pNode as a start or end node.
	 * Note that layout() needs to be called before the change has effect.
	 *
	 * @param pNode
	 *            The target node.
	 */
	 ...
	 * @param pEnd
	 *            The end node.
	 * @return True if and only if there is an edge of type pType that starts at
	 *         node pStart and ends at node pEnd.
	 */
	...
	public void removeNode(Node pNode) {
		...
		// {
		// ArrayList<ChildNode> children = new
		// ArrayList<ChildNode>(((ParentNode) pNode).getChildren());
		// //We create a shallow clone so deleting children does not affect the
		// loop
	...
	/**
	 * @param pElement
	 *            The element we want to check is in the graph.
	...
	public Rectangle2D getBounds() {
		...
		if (bounds == null) {
			...
			return new Rectangle2D.Double(bounds.getX(), bounds.getY(), bounds.getWidth() + AbstractNode.SHADOW_GAP,
					bounds.getHeight() + AbstractNode.SHADOW_GAP);
		}
	}
	...
	/**
	 * Restores a root node to this graph. It is assume that restoring the node
	 * is a valid operation, and that the node properties store its proper
	 * position. The root node restored should be already linked to all its
	 * children (for instance by a deserializer). This operation does not
	...
	/**
	 * Adds an edge to this graph. This method does no validation, but triggers
	 * a notification.
	 *
	 * @param pEdge
	 *            the edge to insert.
	...
	/**
	 * Restores an edge to this graph. It is assume that restoring the edge is a
	 * valid operation. This operation does not trigger any notifications. Note
	 * that we pass the edge's two endpoint because these values are not
	 * serializable due to the absence of set methods for them in the Edge
	 * interface.
	 *
	 * @param pEdge
	 *            The edge to restore
	...
	/**
	 * Checks whether it is legal to connect pNode1 to pNode2 through pEdge
	 ...
	 *            The second node
	 * @param pPoint2
	 *            The point where the edge is supposed to be terminated
