...
public class GraphPanel extends JPanel {
    ...
    /**
     * Constructs the panel, assigns the graph to it, and registers the panel as
     * a listener for the graph.
     *
     * @param pGraph   The graph managed by this panel.
     * @param pSideBar the Side Bar which contains all of the tools for nodes and
     *                 edges.
     */
    ...
    /**
     * Indicate to the GraphPanel that is should consider all following
     * operations on the graph to be part of a single conceptual one.
     */
    ...
    /**
     * Indicate to the GraphPanel that is should stop considering all following
     * operations on the graph to be part of a single conceptual one.
     */
    ...
    /**
     * Clears the selection list and adds all the root nodes and edges to it.
     * Makes the selection tool the active tool.
     */
...
//					Math.max(bounds.getMaxY() / aZoom, graphBounds.getMaxY())));
        ...
        for (GraphElement selected : aSelectedElements) {
            if (!aGraph.contains(selected)) {
                ...
                Rectangle2D grabberBounds = selected.getBounds();
        ...
        if (aDragMode == DragMode.DRAG_RUBBERBAND) {
            ...
            Rectangle2D.Double lasso = new Rectangle2D.Double(Math.min(x1, x2), Math.min(y1, y2), Math.abs(x1 - x2),
                    Math.abs(y1 - y2));
            g2.draw(lasso);
            g2.setColor(GraphPanel.GRABBER_FILL_COLOR_TRANSPARENT);
            g2.fill(lasso);
            g2.setColor(oldColor);
        }
    }
    ...
    public static void drawGrabber(Graphics2D pGraphics2D, double pX, double pY) {
        ...
        pGraphics2D.setColor(GraphPanel.GRABBER_COLOR);
        pGraphics2D.drawRect((int) (pX - size / 2), (int) (pY - size / 2), size, size);
        pGraphics2D.setColor(GraphPanel.GRABBER_FILL_COLOR);
     ...
     * @return whether or not there is a problem with switching to the selection
     * tool.
     */
    public boolean switchToSelectException(Node pNode) {
        // if(pNode instanceof PackageNode || pNode instanceof
        // ImplicitParameterNode || pNode instanceof ObjectNode)
        // {
        // return true;
        // }
        return false;
    ...
    private class GraphPanelMouseListener extends MouseAdapter {
        ...
        private void setSelection(GraphElement pElement) {
            ...
            aSelectedElements.add(pElement); // Necessary to make a parent node
            // the last node selected so it
            // can be edited.
        }
        ...
        private boolean hasSelectedParent(Node pNode) {
            if (pNode == null) {
                return false;
            } else // else if( pNode instanceof ChildNode )
// {
// return hasSelectedParent( ((ChildNode)pNode).getParent() );
// }
                return aSelectedElements.contains(pNode);
        }
        ...
        private void handleNodeCreation(MouseEvent pEvent) {
            ...
            } else // Special behavior, if we can't add a node, we select any
         ...
         * aSideBar.getSelectedTool(), except if the mouse points to an existing
         * node, in which case defaults to select mode because it's likely the
         * user wanted to select the node and forgot to switch tool. The only
         * exception is when adding children nodes, where the parent node
         * obviously has to be selected.
         */
        private GraphElement getTool(MouseEvent pEvent) {
            ...
            if (tool != null && tool instanceof Node) {
                if (selected != null && selected instanceof Node) {
                    // if(!(tool instanceof ChildNode && selected instanceof
                    // ParentNode ))
                    // {
                    aSideBar.setToolToBeSelect();
                    tool = null;
        ...
        public void mousePressed(MouseEvent pEvent) {
            ...
            if (pEvent.getClickCount() > 1 || (pEvent.getModifiers() & InputEvent.BUTTON1_MASK) == 0) // double/right
            // click
            {
        ...
        public void mouseReleased(MouseEvent pEvent) {
            ...
            if (aDragMode == DragMode.DRAG_RUBBERBAND) {
                ...
                if (mousePoint.distance(aMouseDownPoint) > GraphPanel.CONNECT_THRESHOLD
                        && aGraph.addEdge(newEdge, aMouseDownPoint, mousePoint)) {
                    setModified(true);
        ...
        public void mouseDragged(MouseEvent pEvent) {
            ...
            if (aDragMode == DragMode.DRAG_MOVE && aSelectedElements.getLastNode() != null) {
                ...
                for (GraphElement selected : aSelectedElements) {
                    ...
                    // if (!aSelectedElements.parentContained(n)) // parents are
                    // responsible for translating their children
                    // {
                    // n.translate(dx, dy);
                    // }
                    // }
                ...
                // Edges need to be added too when highlighted, but only if both
                // their endpoints have been highlighted.
                for (Edge edge : aGraph.getEdges()) {
                    if (!isCtrl && !lasso.contains(edge.getBounds())) {
                        aSelectedElements.remove(edge);
                    } else if (lasso.contains(edge.getBounds())) {
                        if (aSelectedElements.transitivelyContains(edge.getStart())
                                && aSelectedElements.transitivelyContains(edge.getEnd())) {
                            aSelectedElements.add(edge);
                        }
                    }
                }
    ...
    private class PanelGraphModificationListener implements GraphModificationListener {
        ...
        @Override
        public void propertyChanged(Graph pGraph, GraphElement pElement, String pProperty, Object pOldValue,
                                    Object pNewValue) {
            aUndoManager.add(PropertyChangeTracker.createPropertyChangeCommand(pGraph, pElement, pProperty, pOldValue,
                    pNewValue));
        }
    }
}
